...
This prints two addresses. Copy the second one (`wallet address : addr_test1...`) — you will paste it into the faucet next. The seed is saved to `demo/.wallet.seed` (gitignored).
...
## 4. Install the web app and set the env
...
<AccordionGroup>
  <Accordion title="`next: command not found` when I run npm run dev">
    You're in `demo-nextjs/` but never ran `npm install` there. Run it from `demo-nextjs/`, then `npm run dev` again.
  </Accordion>
  <Accordion title="`Missing script: dev` at repo root">
    `npm run dev` belongs in `demo-nextjs/`, not the SDK root. `cd demo-nextjs` first.
  </Accordion>
  ...
  <Accordion title="Hero price shows --">
    Kupo endpoint hiccup. Retry the page; if it keeps failing, run `npx ts-node examples/read-price.ts` from the SDK root for a stack trace.
  </Accordion>
  ...
  <Accordion title="`Cannot convert undefined to a BigInt`">
    `demo/artifacts.json` is missing. Re-run `npm run demo:setup` from the SDK root.
  </Accordion>
</AccordionGroup>
